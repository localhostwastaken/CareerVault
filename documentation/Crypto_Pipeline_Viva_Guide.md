...
> **Branch:** `ly-final-hardening`. Every `file:line` link points at the code as of commit `e6a5a42`. This guide changed no code, so the links still hold.
...
sequenceDiagram
    ...
    API->>API: Merkle tree over unanchored hashes, sorted pairs
...
- `MerkleService.runBatch` ([merkle.service.ts:52-150](../server/src/modules/merkle/merkle.service.ts#L52-L150)) selects documents that are `ISSUED`, have a hash, and have no proof yet ([merkle.service.ts:61-70](../server/src/modules/merkle/merkle.service.ts#L61-L70)). They are ordered by `issuedAt`, then `id`.
...
- An in-process flag stops the cron and a manual run overlapping ([merkle.service.ts:35-38](../server/src/modules/merkle/merkle.service.ts#L35-L38)).

**Step 8: anchor on Polygon Amoy.**
- **Idempotency first.** The batch asks the chain whether this root already exists, and only sends a transaction if it doesn't ([merkle.service.ts:80-87](../server/src/modules/merkle/merkle.service.ts#L80-L87)).
...
| 6 | Revocation status (`status`) | DB status and `revokedAt` (authoritative, R7), then `expiresAt`. The on-chain `isRevoked` is only a note, and is skipped when step 5 found the chain down. | [verification.service.ts:202-226](../server/src/modules/verification/verification.service.ts#L202-L226) |

**Verdict** ([verification.service.ts:228-238](../server/src/modules/verification/verification.service.ts#L228-L238)): `REVOKED` wins over `EXPIRED`. Next comes `VERIFIED` (checks 1–4 pass and the anchor passes), then `VERIFIED_PENDING_ANCHOR` (checks 1–4 pass and the anchor is pending), then `INVALID`. An unknown or malformed hash is `NOT_FOUND` ([verification.service.ts:68-75](../server/src/modules/verification/verification.service.ts#L68-L75)).

Every public verification writes a COMPLIANCE-tier audit row ([verification.service.ts:343-365](../server/src/modules/verification/verification.service.ts#L343-L365)). The anonymous view shows only a per-type allow-list of fields. A salary lookup, for example, hides every amount, the PAN/UAN and the holder's name ([verification.service.ts:278-314](../server/src/modules/verification/verification.service.ts#L278-L314), [public-fields.ts:10-60](../server/src/modules/document/public-fields.ts#L10-L60)).
...
|---|---|---|
| Master key | `KMS_MASTER_KEY` is base64 that must decode to exactly 32 bytes, and it's required in production. A corrupt key file is **never** regenerated over, since that would orphan every wrapped key. | [master-key.ts:9-53](../server/src/services/key-management/master-key.ts#L9-L53), [env.validation.ts:105-138](../server/src/config/env.validation.ts#L105-L138) |
...
|---|---|
| `documentHash` | The public `/verify/hash/:hash` looks it up by equality, and the Merkle batch uses it as the leaf. Ciphertext can't serve either: the extension refuses any filter on an encrypted field ([field-encryption.extension.ts:227-242](../server/src/prisma/encryption/field-encryption.extension.ts#L227-L242)). It is salted and one-way, and printed on the PDF anyway. |
| `signingPublicKeyPem` | A public key. |
...
**How it's wired.**
- Prisma 7 removed `$use` middleware. The supported replacement is a **query extension**, `$extends({ query })`, and `PrismaService` is provided already extended ([prisma.module.ts:12-22](../server/src/prisma/prisma.module.ts#L12-L22)). Every service keeps reading and writing plaintext.
- Query extensions don't see nested reads, so the extension keys on **field names**, which are unique to these two models. A `Document` returned inside an `include` is therefore still decrypted ([field-encryption.extension.ts:13-22](../server/src/prisma/encryption/field-encryption.extension.ts#L13-L22), [field-encryption.extension.ts:273-299](../server/src/prisma/encryption/field-encryption.extension.ts#L273-L299)).
- Nested writes of encrypted fields are refused ([field-encryption.extension.ts:188-208](../server/src/prisma/encryption/field-encryption.extension.ts#L188-L208)).
- A value that is already an envelope isn't sealed twice ([field-encryption.extension.ts:152-170](../server/src/prisma/encryption/field-encryption.extension.ts#L152-L170)).
...
- **Anchoring** simply retries next run. The batch writes nothing to the DB until the chain answers ([merkle.service.ts:80-89](../server/src/modules/merkle/merkle.service.ts#L80-L89)).
...
Not one that shows as anchored in *our* registry.
...
- The upgrade path (RSA-3072 or ECDSA P-256) is a KMS driver change; the statement format doesn't change.
...
`DELETE /users/me` ([user.service.ts:46-117](../server/src/modules/user/user.service.ts#L46-L117)):
- nulls the **salt** on all the holder's documents;
- scrubs content of non-issued documents and **every version snapshot**;
