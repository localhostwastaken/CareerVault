...
> **Branch:** `ly-final-hardening`. Every `file:line` link points at the code as of commit `96f685f`, after Task 9 added strict R10 reads, the batch integrity gate and complete GDPR erasure, and its first review round. The docs commits after it change no code, so the links still hold.
...
- `MerkleService.runBatch` ([merkle.service.ts:54-155](../server/src/modules/merkle/merkle.service.ts#L54-L155)) selects documents that are `ISSUED`, have a hash, still have a salt (so an erased document is never a candidate), and have no proof yet ([merkle.service.ts:63-74](../server/src/modules/merkle/merkle.service.ts#L63-L74)). They are ordered by `issuedAt`, then `id`.
- **The integrity gate** runs before any tree is built ([merkle.service.ts:75](../server/src/modules/merkle/merkle.service.ts#L75), [integrity-gate.ts:27-63](../server/src/modules/merkle/integrity-gate.ts#L27-L63)):
...
- **Idempotency first.** The batch asks the chain whether this root already exists, and only sends a transaction if it doesn't ([merkle.service.ts:85-92](../server/src/modules/merkle/merkle.service.ts#L85-L92)).
...
**Step 9: store the per-document proof.** A single DB transaction ([merkle.service.ts:94-130](../server/src/modules/merkle/merkle.service.ts#L94-L130)):
...
Every check recomputes from scratch. No stored "is valid" flag is trusted ([verification.service.ts:58-60](../server/src/modules/verification/verification.service.ts#L58-L60)). These stored values *are* trusted:
- the document's pinned `signingPublicKeyPem`, a plaintext column;
- the organisation's `publicKeyPem`, also plaintext, as the fallback when a row has no pinned key ([verification.service.ts:353-354](../server/src/modules/verification/verification.service.ts#L353-L354));
- the DB `status` and `revokedAt`, which decide revocation (R7, [verification.service.ts:220](../server/src/modules/verification/verification.service.ts#L220)).

See Q14 for what that means for a tampered database.

| # | Check (`key`) | What it recomputes | Code |
|---|---|---|---|
| 1 | Document on record (`exists`) | Status is `ISSUED`, `ANCHORED`, `REVOKED` or `EXPIRED`. A rejected draft's stale hash doesn't pass. | [verification.service.ts:139-151](../server/src/modules/verification/verification.service.ts#L139-L151) |
| 2 | Content integrity (`integrity`) | `hashDocument(contentJson, salt) === documentHash`. After GDPR erasure, an issued document has no salt and its content is `{}`. The check then fails with "The holder exercised their right to erasure; the original content no longer exists.", and the response carries `erased: true` and no document (Q22). | [verification.service.ts:153-169](../server/src/modules/verification/verification.service.ts#L153-L169), [is-erased.ts:8-23](../server/src/modules/verification/is-erased.ts#L8-L23) |
| 3 | Issuer signature (`issuerSignature`) | RS256 over the MANAGER statement, rebuilt from the stored `signerMemberId`. Checked against the document's pinned `signingPublicKeyPem`, falling back to the org key. | [verification.service.ts:171-187](../server/src/modules/verification/verification.service.ts#L171-L187), [verification.service.ts:341-366](../server/src/modules/verification/verification.service.ts#L341-L366) |
| 4 | Approver signature (`approverSignature`) | The same, with role `HR` and `approverMemberId`. | [verification.service.ts:188-201](../server/src/modules/verification/verification.service.ts#L188-L201) |
...
| 6 | Revocation status (`status`) | DB status and `revokedAt` (authoritative, R7), then `expiresAt`. The on-chain `isRevoked` is only a note, and is skipped when step 5 found the chain down. | [verification.service.ts:217-243](../server/src/modules/verification/verification.service.ts#L217-L243) |

**Verdict** ([verification.service.ts:245-255](../server/src/modules/verification/verification.service.ts#L245-L255)): `REVOKED` wins over `EXPIRED`. Next comes `VERIFIED` (checks 1–4 pass and the anchor passes), then `VERIFIED_PENDING_ANCHOR` (checks 1–4 pass and the anchor is pending), then `INVALID`. An unknown or malformed hash is `NOT_FOUND` ([verification.service.ts:72-86](../server/src/modules/verification/verification.service.ts#L72-L86)). An erased document goes through the same verdict logic, so it reads `INVALID` (or `REVOKED`/`EXPIRED`); `erased: true` is what tells a verifier why. A record whose stored fields can't be read, because an envelope fails authentication or strict mode refuses plaintext, also reads `INVALID`, with no document and one integrity check saying so. A bulk request answers for its other hashes as normal ([verification.service.ts:72-86](../server/src/modules/verification/verification.service.ts#L72-L86), [verification.service.ts:392-419](../server/src/modules/verification/verification.service.ts#L392-L419)).

Every public verification writes a COMPLIANCE-tier audit row ([verification.service.ts:368-390](../server/src/modules/verification/verification.service.ts#L368-L390)). The anonymous view shows only a per-type allow-list of fields. A salary lookup, for example, hides every amount, the PAN/UAN and the holder's name ([verification.service.ts:303-339](../server/src/modules/verification/verification.service.ts#L303-L339), [public-fields.ts:10-60](../server/src/modules/document/public-fields.ts#L10-L60)). For an erased document, the lookup shows nothing about the holder: `document` is `null` and a revocation's free-text reason is withheld ([verification.service.ts:222-223](../server/src/modules/verification/verification.service.ts#L222-L223), [verification.service.ts:260-298](../server/src/modules/verification/verification.service.ts#L260-L298)).
...
- **One data key per row payload.** Each `encrypt()` call makes one KMS call, however many fields that payload seals ([field-cipher.ts:65-74](../server/src/services/key-management/field-cipher.ts#L65-L74), [field-encryption.extension.ts:163-186](../server/src/prisma/encryption/field-encryption.extension.ts#L163-L186)). A `create`/`update` payload gets its own key, and so does each entry of a `createMany`. An `updateMany` seals its single payload once, so every row it matches gets the *same* envelope. GDPR erasure's document and version scrubs are examples ([user.service.ts:95-106](../server/src/modules/user/user.service.ts#L95-L106)).
...
|---|---|
| `documentHash` | The public `/verify/hash/:hash` looks it up by equality, and the Merkle batch uses it as the leaf. Ciphertext can't serve either: the extension refuses any filter on an encrypted field ([field-encryption.extension.ts:263-278](../server/src/prisma/encryption/field-encryption.extension.ts#L263-L278)). It is salted and one-way, and printed on the PDF anyway. |
...
- The read walk never enters a **non-encrypted Json column** (`PLAIN_JSON_FIELDS`, pinned to the schema by its spec). A key inside one, such as an AI-extracted skill named `salt`, is caller data and never taken for a field, in either mode ([encrypted-fields.ts:25-41](../server/src/prisma/encryption/encrypted-fields.ts#L25-L41), [field-encryption.extension.ts:328](../server/src/prisma/encryption/field-encryption.extension.ts#L328)).
- Query extensions don't see nested reads, so the extension keys on **field names**, which are unique to these two models. A `Document` returned inside an `include` is therefore still decrypted ([field-encryption.extension.ts:14-23](../server/src/prisma/encryption/field-encryption.extension.ts#L14-L23), [field-encryption.extension.ts:309-345](../server/src/prisma/encryption/field-encryption.extension.ts#L309-L345)).
- Nested writes of encrypted fields are refused ([field-encryption.extension.ts:224-244](../server/src/prisma/encryption/field-encryption.extension.ts#L224-L244)).
- A value that is already an envelope isn't sealed twice ([field-encryption.extension.ts:188-206](../server/src/prisma/encryption/field-encryption.extension.ts#L188-L206)).
- Raw SQL returns ciphertext.
- **Strict reads (`FIELD_ENCRYPTION_STRICT`).**
  - On: an encrypted field holding any non-null value that isn't an envelope makes the read throw `PlaintextFieldError`. The error names the field, never the value ([field-encryption.extension.ts:27-42](../server/src/prisma/encryption/field-encryption.extension.ts#L27-L42), [field-encryption.extension.ts:338-344](../server/src/prisma/encryption/field-encryption.extension.ts#L338-L344)).
...
- The strict-mode e2e plants a self-signed plaintext row with raw SQL. It asserts that the row is refused on read, that its public lookup fails closed, and that `/merkle/run` skips it while the genuine document anchors ([strict-encryption.e2e-spec.ts:211-284](../server/test/strict-encryption.e2e-spec.ts#L211-L284)).
...
| **Batch anchoring** | One transaction per document | Constant cost per batch (one `anchorRoot`), proofs of at most `⌈log2 N⌉` hashes, and no individual document hash published by the batch. | [merkle.service.ts:54-155](../server/src/modules/merkle/merkle.service.ts#L54-L155) |
...
- **What they'd try.** They can't read R10 fields, and the org's private key isn't in the database, so they can't sign anything with it directly. But verification trusts the pinned public key stored in the row ([verification.service.ts:353-354](../server/src/modules/verification/verification.service.ts#L353-L354)). So they build a **self-consistent forged document**: plaintext content and salt, a matching hash, their own public key and their own signatures, in an `ISSUED` row with no proof, which the batch selects ([merkle.service.ts:63-74](../server/src/modules/merkle/merkle.service.ts#L63-L74)).
- **Production (`FIELD_ENCRYPTION_STRICT=true`, set in `render.yaml`).**
  - Without the KEK they can't make an envelope, so the planted content can only be plaintext, and strict reads refuse it ([field-encryption.extension.ts:338-344](../server/src/prisma/encryption/field-encryption.extension.ts#L338-L344)).
  - Every read of the row is refused. Its public lookup fails closed as `INVALID`, with no document and a check saying the stored content failed integrity checks, and it writes a `DOCUMENT_CHECK_FAILED` audit row ([verification.service.ts:72-86](../server/src/modules/verification/verification.service.ts#L72-L86), [verification.service.ts:392-419](../server/src/modules/verification/verification.service.ts#L392-L419)). A bulk request answers for its other hashes as normal.
  ...
  - The strict-mode e2e proves each step ([strict-encryption.e2e-spec.ts:211-284](../server/test/strict-encryption.e2e-spec.ts#L211-L284)).
- **Dev (strict mode off, the default).** The extension returns the plaintext unchanged ([field-encryption.extension.ts:327-339](../server/src/prisma/encryption/field-encryption.extension.ts#L327-L339)). The row verifies as `VERIFIED_PENDING_ANCHOR`, and the next batch anchors it (L13).
...
- **Anchoring** simply retries next run. The batch writes nothing to the DB until the chain answers ([merkle.service.ts:85-94](../server/src/modules/merkle/merkle.service.ts#L85-L94)).
...
- **But the pinned public key is itself a DB column** ([verification.service.ts:353-354](../server/src/modules/verification/verification.service.ts#L353-L354)). So a DB writer can build a forged document signed with **their own** key, with plaintext content and salt.
  ...
  - **Dev (strict mode off):** R10 reads it as legacy plaintext ([field-encryption.extension.ts:327-339](../server/src/prisma/encryption/field-encryption.extension.ts#L327-L339)). It verifies as pending, and the next batch anchors it (L13).
...
- Its integrity check reads "The holder exercised their right to erasure; the original content no longer exists." ([verification.service.ts:143](../server/src/modules/verification/verification.service.ts#L143), [is-erased.ts:8-23](../server/src/modules/verification/is-erased.ts#L8-L23)).
